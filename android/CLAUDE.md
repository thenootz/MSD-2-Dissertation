...
- **`overlay/OverlayManager.kt`** — `WindowManager` overlay (currently unused by the active pipeline; kept for a future annotation-overlay feature). The dashboard does **not** request `SYSTEM_ALERT_WINDOW` before starting a capture session.
- **`permissions/PermissionManager.kt`** — overlay / POST_NOTIFICATIONS / MediaProjection permission helpers. The overlay helpers are retained but unused by the active flow.
