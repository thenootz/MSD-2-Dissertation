...
Write-Host "Building for Android (release mode)..." -ForegroundColor Green
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "Library sizes:" -ForegroundColor Cyan
    Get-ChildItem "$jniLibsPath" -Recurse -Filter "*.so" | ForEach-Object {
        $size = [math]::Round($_.Length / 1MB, 2)
        Write-Host "  $($_.FullName): $size MB"
    }
    
    Write-Host "Build complete! Now open the Android project in Android Studio." -ForegroundColor Green
